# Docs Index

Super Lobster OS documentation is organized to support both quick discovery and deeper architectural reading.

## Start Here

- [Main README](../README.md)
- [Chinese README](../README.zh-CN.md)
- [Japanese README](../README.ja.md)
- [Spanish README](../README.es.md)
- [Korean README](../README.ko.md)

## Core Docs

- [Architecture](./ARCHITECTURE.md)
- [Comparison](./COMPARISON.md)
- [Roadmap](./ROADMAP.md)

## Visual Assets

- [Hero Graphic](./assets/lobster-hero.svg)
- [Mission Map](./assets/mission-map.svg)

These SVGs are generated from a browser-side layout pipeline using [`@chenglou/pretext`](https://github.com/chenglou/pretext), so README typography can be regenerated instead of hand-positioned.

## Reading Order

1. Start with the main README for product positioning.
2. Read Architecture to understand the runtime model.
3. Read Roadmap for what comes next.
4. Read Comparison to understand market position and inspiration boundaries.
